...
/**
 * @desc    Submit form (change status from Draft to Submitted)
...
exports.verifyFormSubmission = async (req, res) => {
  ...
  try {
    ...
    if (submission.status !== 'Submitted') {
      return res.status(400).json({
        success: false,
        message: 'Form can only be verified from Submitted status'
      });
    }

    // Add verification to approval workflow
    ...
    await AuditLog.createLog({
      user: req.user._id,
      action: 'verify_form',
      description: `Verified form submission: ${submission.title}`,
      resourceType: 'form_submission',
      resourceId: submission._id,
      resourceModel: 'FormSubmission',
      status: 'success',
    ...
    res.status(500).json({
      success: false,
      message: 'Server error while verifying form',
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
  }
};
...
exports.approveFormSubmission = async (req, res) => {
  ...
  try {
    ...
    // Check if form is in verified status
    if (submission.status !== 'Verified') {
      return res.status(400).json({
        success: false,
        message: 'Form can only be approved from Verified status'
      });
    }

    // Add approval to approval workflow
    submission.approvalWorkflow.push({
      step: 'approval',
      status: 'approved',
      userId: req.user._id,
    ...
    await AuditLog.createLog({
      user: req.user._id,
      action: 'approve_form',
      description: `Approved form submission: ${submission.title}`,
      resourceType: 'form_submission',
      resourceId: submission._id,
      resourceModel: 'FormSubmission',
      status: 'success',
    ...
    res.status(500).json({
      success: false,
      message: 'Server error while approving form',
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
  }
};
